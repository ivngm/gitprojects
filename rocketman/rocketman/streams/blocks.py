...
class LinkValue(blocks.StructValue):
    """Add aditional logic for our links"""
    def url(self) -> str:
...
class Link(blocks.StructBlock):
    ...
    def clean(self, value):
        ...
        if internal_page and external_link:
            errors["internal_page"]=ErrorList(["Both of this Fields cannot be filled. Please select or enter only one option"])
            errors["external_link"]=ErrorList(["Both of this Fields cannot be filled. Please select or enter only one option"])
...
class ImageAndTextBlock(blocks.StructBlock):
        image=ImageChooserBlock(
            help_text="Image will automagically cropped to 786px by 552px",
        )
